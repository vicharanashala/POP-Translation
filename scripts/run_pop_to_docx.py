...
def main():
    parser = argparse.ArgumentParser(
        description="POP Translation Pipeline: source PDF -> Gemini 3.1 Pro HTML -> images injected -> final DOCX"
    )
    ...
    log("[4/5] Building combined HTML for DOCX...")

    combined_html_path = build_combined_docx_html(
        page_pdf_files=page_pdf_files,
    ...
    log(f"Combined HTML: {combined_html_path}")

    # 5. Convert to DOCX
    log("[5/5] Converting combined HTML to DOCX...")
